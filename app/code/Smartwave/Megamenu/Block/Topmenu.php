...
class Topmenu extends \Magento\Framework\View\Element\Template
{
    ...
    public function getSubmenuItemsHtml($cat_id,$children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null)
    {
        $products = $this->getProducts($cat_id);
        ...
        $html = '<ul class="subchildmenu">';
       // echo 'url'.$this->getBaseUrl();
        foreach($products as $product_data) {
            $data = $product_data->getData();  
            ...
            if ($data) {
                ...
                $html .= '<a href= "'.$this->getBaseUrl().$url.'">';
                $html .= '<span>'.$name;
                $html .= '</span></a>';
                $html .= '</li>';
            }
    ...
    public function getSubcatItemsHtml($cat_id)
    {
        $products = $this->getProducts($cat_id);
        $products->setOrder('position','ASC');
        $html = '';
    ...
    public function getMegamenuHtml()
    {
        ...
        foreach($categories as $category) {
            ...
            if(!$sw_menu_hide_item) {
                ...
                foreach($children as $sub) {
                    ...
                    $subcategories[$count]['name']    = $sub['name'];
                    $subcategories[$count]['id']      = $sub['entity_id'];
                    $subcategories[$count]['url_key'] = $sub['url_key'].'.html';
                    $count++;
                }
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) {
                    ...
                    if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_left_content || $menu_right_content))) {
                        ...
                        $html .= $this->getSubmenuItemsHtml($category->getId(),$children, 1, $max_level, 12-$menu_left_width-$menu_right_width, $menu_type, $sw_menu_cat_columns);
        ...
        for($i=0;$i<$count;$i++){
            $html .= '<li class="ui-menu-item level0 fullwidth parent">';
            $html .= '<div class="open-children-toggle"></div>';
            $html .= '<a href = "'.$baseUrl.$subcategories[$i]['url_key'].'" class="level-top" >';
            $html .= '<span>'.$subcategories[$i]['name'].'</span></a>';
            $html .= '<div class="level0 submenu">';
            $html .= '<ul class="subchildmenu">';
            $html .= $this->getSubcatItemsHtml($subcategories[$i]['id']);
            $html .= '</ul>';
            $html .= '</div>';
            $html .= '</li>';              
        }
