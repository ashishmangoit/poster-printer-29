...
class SendMailOnOrderSuccess implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if(count($orderIds))
        {
		    ...
		    $shipping_address_data = '<p style="margin: 0;font-size: 14px;font-weight: 600;line-height: 20px;">' . $shipping_address['firstname'] . ' ' . $shipping_address['lastname'] . '<br/>'.$shipping_address['company'].'<br>' . $shipping_address['street'] . '<br/>' . $shipping_address['city'].', '.$shipping_address['region'].', '.$shipping_address['postcode'] . '<br/>'.$shipping_country_name.'
		    ...
		    $billing_address_data = '<p style="margin: 0;font-size: 14px;font-weight: 600;line-height: 20px;">' . $billingAddress['firstname'] . ' ' . $billingAddress['lastname'] . '<br/>'.$billingAddress['company'].'<br>' . $billingAddress['street'] . '<br/>' . $billingAddress['city'].', '.$billingAddress['region'].', '.$billingAddress['postcode'] . '<br/>'.$billingcountry_name.'
		                    <br>T : ' . $billingAddress['telephone'] . '</p>';
